   ...
   # ex. gcc_gcc_gcc_gcc
    ...
    for wrkld in workloads:
        ...
        if wrkld == 'perlbench':
            ...
            process.executable = bench_path+wrkld + x86_suffix
            process.cmd = [process.executable] + ['--fixed', '0', '--mean', '325', '--num', '45000', '--sd', '200', '--seed', '0', 'bombesin.hmm']
        elif wrkld == 'GemsFDTD':
            process.executable = bench_path+wrkld + x86_suffix
            process.cmd = [process.executable] 
            ...
            process.executable =bench_path+ wrkld + x86_suffix
            process.cmd = [process.executable] + ['-d', 'foreman_test_encoder_baseline.cfg']
        elif wrkld == 'tonto':
            process.executable = bench_path+wrkld + x86_suffix
            process.cmd = [process.executable] 
        elif wrkld == 'lbm':
            process.executable = bench_path+wrkld + x86_suffix
            process.cmd = [process.executable] + ['20', 'reference.dat', '0', '1', '100_100_130_cf_a.of']
        elif wrkld == 'omnetpp':
            process.executable =bench_path+ wrkld + x86_suffix
...
if options.benchmark:
   ...
   # print("  JWL SE DEBUG: after get_benchmark() ", multiprocesses)
else:
