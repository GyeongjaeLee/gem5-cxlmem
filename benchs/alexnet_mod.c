...
//JH dummylayers
layer dummy_layer (layer input, int filter, int f_size, int stride, int pad);
layer dummy_pool_layer(layer input, int kernel, int stride, int pad);
layer dummy_fc_layer(layer input, int filter);
...
//JH
	// total topology
	/*
	conv1 = dummy_layer(input, 96, 11, 4, 0);
	pool1 = dummy_pool_layer(conv1, 3, 2, 0);
	conv2 = dummy_layer(pool1, 256, 5, 1, 2 );
	pool2 = dummy_pool_layer(conv2, 3, 2, 0);
	conv3 = dummy_layer(pool2, 384, 3, 1, 1);
	conv4 = dummy_layer(conv3, 384, 3, 1, 1);
	conv5 = dummy_layer(conv4, 256, 3, 1, 1);
	pool5 = dummy_pool_layer(conv5, 3, 2, 0);
	fc6 = dummy_fc_layer(pool5, 4096);
	fc7 = dummy_fc_layer(fc6, 4096);
	fc8 = dummy_fc_layer(fc7, 1000);
*/
	//softmax 
